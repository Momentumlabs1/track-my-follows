...
// Success celebration view — used for both web and native purchase success
...
// Native purchase success overlay — shown when RevenueCat native paywall completes a purchase
function NativePurchaseSuccessSheet() {
  const { nativePurchaseSuccess, clearNativePurchaseSuccess } = useSubscription();

  if (!nativePurchaseSuccess) return null;
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <SuccessView onClose={clearNativePurchaseSuccess} />
      </motion.div>
    </AnimatePresence>
  );
}
...
export function PaywallSheet() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      // Web fallback purchase flow (native uses RevenueCat's own paywall now)
      if (isNativeApp()) {
  ...
  return (
    <>
      {/* Native purchase success overlay (triggered by onRevenueCatPurchase callback) */}
      <NativePurchaseSuccessSheet />

      {/* Web custom paywall (only opens on web, never on native) */}
      <AnimatePresence>
        {isPaywallOpen && (
          <>
            <motion.div
